...
def run_net(rtc, otp_dir):
  pnte = PTNspkTrainEvalModel(args.dataset, rtc)
  log.INFO("Starting the PyTorch Non-spiking model training...")
  loss_history = pnte.train_model(args.epochs, otp_dir)
...
def setup_logging(rtc, otp_dir):
  log.configure_log_handler(
      "%s/pytorch_nspk_training_evaluation_%s_%s.log"
...
def setup_otp_dir(rtc):
  if args.meta_cfg_dir:
    otp_dir = drc._results_path + (
        "/pytorch_nspk_train_eval/{0}/{1}/config_{2}/".format(
        rtc.PYTORCH_MODEL_NAME, args.meta_cfg_dir, rtc.CONFIG))
  else:
    otp_dir = drc._results_path + (
        "/pytorch_nspk_train_eval/{0}/config_{1}/".format(rtc.PYTORCH_MODEL_NAME,
        rtc.CONFIG))
...
if __name__ == "__main__":
  ...
  if not args.is_all_combs:
    ...
    all_combs = exu.get_combinations_for_lsnn_nspk(EXC)
    RTC.CONFIG = 0
